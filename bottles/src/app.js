...
const shattered = new Shatter('jameson.png');
    ...
    const app = {
        init: function() {
            ...
            imgs.forEach((item, idx) => {
                ...
                item.addEventListener('click', app.clickBottle);
...
*/            
            ...
            for (let deg=0; deg< 360; deg+=step) {
                x = (Math.cos(deg * d2r) * rad) + offsetX;
                y = (Math.sin(deg * d2r) * rad) + offsetY;
                app.ctx.moveTo(offsetX, offsetY);
        ...
        setup: function() {
            ...
            app.config.bottles.forEach((item, idx) => {
                ...
                app.ctx.drawImage(app.state.images[idx], 
                    0, 0, app.state.bottles[item].width, app.state.bottles[item].height, 
                    xpix, (row * (app.state.maxHeight * scale)) + ((app.state.maxHeight * scale) - (app.state.bottles[item].height * scale)), app.state.bottles[item].width * scale, app.state.bottles[item].height * scale);
        ...
        clickBottle: function(e) {
            ...
            let bottle = e.target.id;
            app.shatter(app.state.bottles[e.target.id], 25);
        ...
        config: {
            ...
            particles: {
                ...
                size: {                 // range for particle size
                    x: [10, 30],
                    y: [10, 30]
                },
                initSpeed: 15,          // power of explosion
                gravity: 0.3,          // defines how fast particles fall
                drag: 0.08,             // how wide is explosion
                terminalVelocity: 3,    // fastest particles can fall
                flipSpeed: 0.1,
            },
            scale: 0.5
        ...
        state: {
            ...
            particles: [],
            bottles: {
                    "absolute": {"width": 144, "height": 446, "src": "absolute" },
                    "amaro": {"width": 136, "height": 518, "src": "amaro" },
                    "baileys": {"width": 168, "height": 414, "src": "baileys" },
                    "becherovka": {"width": 162, "height": 444, "src": "becherovka" },
                    "beefeater": {"width": 142, "height": 518, "src": "beefeater" },
                    "chivasregal": {"width": 174, "height": 450, "src": "chivasregal" },
                    "contreau": {"width": 158, "height": 380, "src": "contreau" },
                    "curacao": {"width": 248, "height": 420, "src": "curacao" },
                    "havanaclub": {"width": 122, "height": 506, "src": "havanaclub" },
                    "jackdaniels": {"width": 184, "height": 516, "src": "jackdaniels" },
                    "jagermeister": {"width": 182, "height": 400, "src": "jagermeister" },
                    "jameson": {"width": 144, "height": 542, "src": "jameson" },
                    "kahlua": {"width": 144, "height": 478, "src": "kahlua" },
                    "malibu": {"width": 148, "height": 506, "src": "malibu" },
                    "martell": {"width": 160, "height": 524, "src": "martell" },
                    "olmeca": {"width": 142, "height": 434, "src": "olmeca" },
                    "pernod": {"width": 144, "height": 510, "src": "pernod" },
                    "sambuca": {"width": 126, "height": 500, "src": "sambuca" }
            }

        },
        shatter: async function(bottle, num=15) {
            console.dir(bottle);
            ...
            const vshatter = new Shatter(bottle.img.src);
            ...
            assembled.style.left = bottle.img.offsetLeft + 'px';
            assembled.style.top = bottle.img.offsetTop + 'px';

            bottle.img.style.visibility = 'hidden';

            let step = 360 / results.length;
            let d2r = Math.PI / 180;
            ...
            results.forEach((res, i) => {
                ...
                clone.setAttribute('style', `position: absolute; top: ${res.y}px; left: ${res.x}px;animation-duration:${~~(Math.random()*3000)+4000}ms; animation-delay:${~~(Math.random()*500)+300}ms;animation-direction:${~~(Math.random()>.5)?'reverse':'forwards'}`);
                assembled.appendChild(clone);
                let x=0, y=0, rad = 500;
                let offsetX = 100, offsetY = 100;

                setTimeout(function() { 
                    let x = (Math.cos((i * step) * d2r) * rad) + offsetX;
                    let y = (Math.sin((i * step) * d2r) * rad) + offsetY;
